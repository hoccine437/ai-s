import os, json, time, math, random, threading, ctypes
...
class JarvisUI:
    def __init__(self, face_path, size=None):
        ...
        self._restore_hotkey_latched = False
        self._camera_hotkey_latched = False

        # Klavye girişinden komutu iletmek için callback — main.py atar
        self.on_text_command = None
        self.on_camera_shortcut = None
        ...
        self.root.bind("<Control-Shift-Q>", lambda e: self.quit_application())
    ...
    def _trigger_camera_shortcut(self):
        self._show_window_impl()
        self.write_log("SYS: Camera shortcut triggered via Ctrl+Alt+C.")
        if not self.on_camera_shortcut:
            self.write_log("ERR: Camera shortcut is not ready yet.")
            return
        threading.Thread(
            target=self.on_camera_shortcut,
            daemon=True
        ).start()

    def _poll_global_hotkeys(self):
        restore_pressed = (
            self._is_vk_pressed(VK_CONTROL)
            and self._is_vk_pressed(VK_MENU)
            and self._is_vk_pressed(VK_J)
        )
        camera_pressed = (
            self._is_vk_pressed(VK_CONTROL)
            and self._is_vk_pressed(VK_MENU)
            and self._is_vk_pressed(VK_C)
        )

        if restore_pressed and not self._restore_hotkey_latched:
            self._restore_hotkey_latched = True
            was_hidden = self.window_hidden
            self._show_window_impl()
            if was_hidden:
                self.write_log("SYS: JARVIS restored via Ctrl+Alt+J.")
        elif not restore_pressed:
            self._restore_hotkey_latched = False

        if camera_pressed and not self._camera_hotkey_latched:
            self._camera_hotkey_latched = True
            self._trigger_camera_shortcut()
        elif not camera_pressed:
            self._camera_hotkey_latched = False

        self.root.after(150, self._poll_global_hotkeys)

    def hide_to_background(self):
        self._run_on_ui_thread(self._hide_to_background_impl)

    def _hide_to_background_impl(self):
    ...
    def show_window(self):
        self._run_on_ui_thread(self._show_window_impl)
    ...
    def _draw(self):
        ...
        c.create_text(W - 16, H - 14, fill=C_DIM, font=("Courier", 8),
                      text="[F4] MUTE  [CTRL+ALT+J] SHOW  [CTRL+ALT+C] CAMERA", anchor="e")
